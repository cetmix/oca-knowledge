...
class TestDocumentUrl(common.TransactionCase):
    ...
    def test_user_saves_own_google_picker_fields(self):
        user = self.users.with_context(no_reset_password=True).create(
            {
                "name": "Picker User",
                "login": "picker_user",
        ...
        user.with_user(user).write(
            {
                ...
                "google_picker_mime_types": "application/pdf",
            }
        )
        user.with_user(user).save_google_picker_access_token("token", 123)
        user.invalidate_recordset()
        self.assertEqual(
            user.google_picker_scope,
            "https://www.googleapis.com/auth/drive.readonly",
        )
        self.assertEqual(user.google_picker_mime_types, "application/pdf")
        self.assertEqual(user.google_picker_access_token, "token")
        self.assertEqual(user.google_picker_expires_date, 123)
